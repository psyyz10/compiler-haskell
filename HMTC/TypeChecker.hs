...
chkCmd env (A.CmdCall {A.ccProc = p, A.ccArgs = es, A.cmdSrcPos = sp}) = do
    ...
    require (t == Void) sp ("Not a procedure; return type is " ++ show t)
...
chkCmd env (A.CmdIf {A.ciCondThens = ecs, A.ciMbElse = mc2,
                     A.cmdSrcPos=sp}) = do
    case mc2 of
        Just c  -> do  ecs'  <- mapM chkEach ecs
                       c'    <- chkCmd env c
                       return (CmdIf {ciCondThens = ecs', ciMbElse = Just c', cmdSrcPos = sp})
        Nothing -> do  ecs'  <- mapM chkEach ecs
                       return (CmdIf {ciCondThens = ecs', ciMbElse = Nothing, cmdSrcPos = sp})  
      where chkEach (e, c) = do  e' <- chkTpExp env e Boolean
                                 c' <- chkCmd env c
                                 return (e', c')
-- T-WHILE
...
infTpExp env e@(A.ExpLitChr {A.elcVal = n, A.expSrcPos = sp}) = do
    n' <- toMTChr n sp
    return (Char,				-- env |- n : Char
            ExpLitChr {elcVal = n', expType = Char, expSrcPos = sp})
-- T-VAR
...
infTpExp env (A.ExpIx {A.eiAry = a, A.eiIx = i, A.expSrcPos = sp}) = do
    (rat, a') <- infRefAryTpExp env a
    i'        <- chkTpExp env i Integer
    let rt = mapRcdType eltType rat
    return (rt, ExpIx {eiAry = a', eiIx = i', expType = rt, expSrcPos = sp})
-- Cond
infTpExp env (A.ExpCond {A.ecCond = ce, A.ecTrue = te, A.ecFalse = fe, A.expSrcPos = sp}) = do
    ce' <- chkTpExp env ce Boolean
    (t1, te') <- infTpExp env te
    (t2, fe') <- infTpExp env te
    if t1 <: t2 then do			                -- s <: Source _
        return (t1, ExpCond {ecCond = ce', ecTrue = te', ecFalse = fe', expType = t1, expSrcPos = sp})
     else do    					-- s /<: Source _
        emitErrD sp ("Expected type \"" ++ show t2 ++ "\", got \"" ++ show t1 ++ "\"")    
        return (t1, ExpCond {ecCond = ce', ecTrue = te', ecFalse = fe', expType = t2, expSrcPos = sp})
